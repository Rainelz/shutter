...
class TableGen:
    ...
    def add_cells_to_table(self, schema, cell_h, cell_w):
        ...
        for cell in schema:
            ...
            cell_istance = TableCell(
                cell_w,
                cell_h,
                fontname,
                self.base_font,
                self.pad_font_min_size,
                color=color,
                text=text,
    ...
    def compose(self, img):
        ...
        schema, cell_h, cell_w = self.make_table_schema(
            n_rows, n_cols, self.width, self.height
        )
...
class TableCell:
    ...
    def add_plain_text(self, img):
        ...
        font = ImageFont.truetype(
            self.fontname, self.base_font + self.pad_font_min_size
        )
        ...
        for phrase in text_out:
            ...
            h = (
                (MAX_H - self.border_width - font.getsize(phrase)[1] * len(text_out))
                / MAX_H
            ) / 2
            draw.text(
                (MAX_W * w, (h * MAX_H) + h_text), phrase, 0, font=font, align="center"
            )
